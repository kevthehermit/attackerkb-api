...
from attackerkb_api import ApiError, AttackerKB


@pytest.fixture
def api():
    """Create an AttackerKB client. Requires ATTACKERKB_API_KEY env var."""
    api_key = os.environ.get("ATTACKERKB_API_KEY")
    if not api_key:
        pytest.skip("ATTACKERKB_API_KEY environment variable not set")
    return AttackerKB(api_key=api_key)


def test_api_fail(monkeypatch):
    monkeypatch.delenv("ATTACKERKB_API_KEY", raising=False)
...
def test_multi_assesment(api):
    result = api.get_assessments(topicId="131226a6-a1e9-48a1-a5d0-ac94baf8dfd2")
    assert len(result) >= 2


def test_single_assesment_fail(api):
    with pytest.raises(ApiError):
        api.get_single_assessment("not a uuid")


def test_single_user_id(api):
